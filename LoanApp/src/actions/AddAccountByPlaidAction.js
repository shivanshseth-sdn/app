...
export const showLoading =(value)=>{
  ...
  return (dispatch) => {
    dispatch({
      type: ACTION_TYPES.ADD_BANK_SHOW_LOADIN,
      payload: value
    });
  }
};
...
export const connectBank = ({authToken,userId,token,accountId,metadata}) => {
  ...
  console.log('add other funding source Postdata JSON='+JSON.stringify({userId,token,accountId,metadata}));
  ...
  return (dispatch) => {
    ...
    .then( (responseJSON) => {
      ...
      dispatch({
        type: ACTION_TYPES.ADD_ACCOUNT_WEB_CALL,
